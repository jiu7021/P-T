...
## 3. SECOM — 실제 fab 라인 공정 센서 데이터 (합성 없음)

| 항목 | 내용 (실측) |
|---|---|
| 배포처 | UCI Machine Learning Repository, dataset 179 |
| URL | https://archive.ics.uci.edu/static/public/179/secom.zip (인증 불필요) |
| 파일 | `secom.data` (5.4 MB), `secom_labels.data`, `secom.names` |
| 규모 | 웨이퍼 1,567장 × 공정 센서 측정값 590개 |
| 라벨 | pass / fail (원본은 -1 / 1). **fail 104장 (6.64%)**, 불균형 1 : 14.1 |
| 기간 | 2008-07-19 ~ 2008-10-17 (89일). **타임스탬프 있음, 시간 순 정렬됨** |
| 결측 | 전체 4.54%. 590개 중 538개 센서에 결측 존재 |
| 기증 | Michael McCann, Adrian Johnston (2008-11-19) |
| 배치 경로 | `data/raw/secom/` |
...
### 이 데이터의 위치 — EDS 측정값을 대체하지 못한다

성격이 다르다. SECOM은 **공정 장비가 남긴 센서값**이고, EDS는 **테스터가 잰 전기
특성값**이다. 다만 **가정이 하나도 들어가지 않는 축**을 하나 만들어 준다:
실제 공정 센서값으로 웨이퍼 합불을 예측하고 어느 센서가 기여하는지 보는 분석.
